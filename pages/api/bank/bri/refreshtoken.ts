...
export default async function handler(
  ...
  // Make sure that the institutionId is only for BRI

  if (institutionId !== 5 && institutionId !== 16) {
    res.status(500).json({ Error: 'Invalid institution id for BRI' });
    throw new Error('Invalid institution id for BRI');
  }
