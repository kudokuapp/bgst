...
export default async function handler(
  ...
  // Make sure that the institutionId is only for Mandiri

  if (institutionId !== 3 && institutionId !== 17) {
    res.status(500).json({ Error: 'Invalid institution id for Mandiri' });
    throw new Error('Invalid institution id for Mandiri');
  }
   ...
   * For Mandiri, the maximum is 6 months back,
   * so we pull 5 months
   */
